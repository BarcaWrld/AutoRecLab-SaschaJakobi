...
def main():
    ...
    summary = df[df['name'].isin(['NDCG', 'Precision']) & (df['k'] == 10)].copy()
    if summary.empty:
        print(f'No metric rows found for dataset: {dataset_id}')
        return

    summary = summary.groupby(['algorithm', 'name', 'k'], as_index=False)['value'].mean()
