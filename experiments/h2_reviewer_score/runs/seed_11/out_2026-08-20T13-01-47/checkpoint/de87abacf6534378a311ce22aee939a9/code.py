...
def main():
    ...
    summary = df[df['k'] == 10].groupby(['algorithm', 'name', 'k'], as_index=False)['value'].mean()
    ...
    ax = pivot.plot(kind='bar', x='algorithm', figsize=(7, 4), rot=0)
    ax.set_ylabel('Score')
    ax.set_title('MovieLens100K Prototype: ItemKNN @10')
    ax.legend(title='Metric')
    plt.tight_layout()

    plot_path = Path(working_dir) / 'movielens100k_prototype_itemknn_metrics.png'
