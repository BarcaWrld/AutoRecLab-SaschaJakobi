...
def _find_results_df(results_map):
    if not results_map:
        return pd.DataFrame()
...
def _ensure_rank_column(df):
    if df.empty or not {'user', 'item', 'score'}.issubset(df.columns):
        return df
...
def main():
    ...
    results_df = _find_results_df(results_map)
    results_df = _ensure_rank_column(results_df)
