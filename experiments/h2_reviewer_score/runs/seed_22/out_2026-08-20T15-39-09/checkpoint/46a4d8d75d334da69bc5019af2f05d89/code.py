...
def main():
    ...
    if not results_df.empty:
        label_candidates = [c for c in results_df.columns if str(c).lower() in {'algorithm', 'algo', 'name'}]
        label_col = label_candidates[0] if label_candidates else results_df.columns[0]

        metric_cols = [c for c in results_df.columns if 'NDCG@10' in str(c) or 'Precision@10' in str(c)]
        if metric_cols:
            plot_df = results_df[[label_col] + metric_cols].copy().set_index(label_col)
            plot_df.plot(kind='bar', ax=ax)
            ax.set_ylabel('Score')
            ax.set_title('MovieLens100K Prototype: ItemKNN')
