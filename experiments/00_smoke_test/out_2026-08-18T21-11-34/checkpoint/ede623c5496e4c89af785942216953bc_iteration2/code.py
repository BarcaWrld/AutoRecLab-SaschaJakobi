...
def main():
    ...
    # Keep the small-sample step, but do NOT convert to implicit before the split.
    pipeline = Pipe(
        Subsample(0.05),
    ...
    # Popularity baseline uses implicit feedback, so convert only after splitting.
    dataset = MakeImplicit(3).process(dataset)
